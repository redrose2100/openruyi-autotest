#!/bin/bash
# ============================================================
# K8s conformance - network networking
#   Run `sonobuoy run --mode non-disruptive-conformance`
#   with an explicit --e2e-focus regex (9 tests).
...
rlJournalStart
    ...
    rlPhaseStartTest "Run sonobuoy conformance (9 tests)"
        ...
        focus='\[sig-network\].*(Networking|HostPort|ServiceCIDR|Ingress)'
        ...
        if [ -z "$detailed" ]; then
            ...
            if [ "$real_failures" -eq 0 ]; then
                rlPass "Conformance focus group passed (9 tests, no unsupported-classified failures)"
            else
