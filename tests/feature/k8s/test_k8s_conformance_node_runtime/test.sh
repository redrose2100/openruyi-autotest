#!/bin/bash
# ============================================================
# K8s conformance - node runtime
#   Run `sonobuoy run --mode non-disruptive-conformance`
#   with an explicit --e2e-focus regex (13 tests).
...
rlJournalStart
    ...
    rlPhaseStartTest "Run sonobuoy conformance (13 tests)"
        ...
        focus='\[sig-node\].*(Container Runtime|RuntimeClass|Kubelet|EtcHosts|Node Lifecycle|Lease)'
        ...
        if [ -z "$detailed" ]; then
            ...
            if [ "$real_failures" -eq 0 ]; then
                rlPass "Conformance focus group passed (13 tests, no unsupported-classified failures)"
            else
