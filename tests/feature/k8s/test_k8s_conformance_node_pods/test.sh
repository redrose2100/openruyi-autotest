#!/bin/bash
# ============================================================
# K8s conformance - node pods
#   Run `sonobuoy run --mode non-disruptive-conformance`
#   with an explicit --e2e-focus regex (30 tests).
...
rlJournalStart
    ...
    rlPhaseStartTest "Run sonobuoy conformance (30 tests)"
        ...
        focus='\[sig-node\].*(Pods?(\s|\[)|PodTemplates|Pods Extended|Pod Generation)'
        ...
        if [ -z "$detailed" ]; then
            ...
            if [ "$real_failures" -eq 0 ]; then
                rlPass "Conformance focus group passed (30 tests, no unsupported-classified failures)"
            else
