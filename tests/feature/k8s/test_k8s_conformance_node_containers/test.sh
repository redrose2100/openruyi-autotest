#!/bin/bash
# ============================================================
# K8s conformance - node containers
#   Run `sonobuoy run --mode non-disruptive-conformance`
#   with an explicit --e2e-focus regex (15 tests).
...
rlJournalStart
    ...
    rlPhaseStartTest "Run sonobuoy conformance (15 tests)"
        ...
        focus='\[sig-node\].*(Containers|InitContainer|Ephemeral|Container Lifecycle|PreStop)'
        ...
        if [ -z "$detailed" ]; then
            ...
            if [ "$real_failures" -eq 0 ]; then
                rlPass "Conformance focus group passed (15 tests, no unsupported-classified failures)"
            else
