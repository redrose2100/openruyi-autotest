#!/bin/bash
# ============================================================
# K8s conformance - storage projected
#   Run `sonobuoy run --mode non-disruptive-conformance`
#   with an explicit --e2e-focus regex (28 tests).
...
rlJournalStart
    ...
    rlPhaseStartTest "Run sonobuoy conformance (28 tests)"
        ...
        if [ -z "$detailed" ]; then
            ...
            if [ "$real_failures" -eq 0 ]; then
                rlPass "Conformance focus group passed (28 tests, no unsupported-classified failures)"
            else
