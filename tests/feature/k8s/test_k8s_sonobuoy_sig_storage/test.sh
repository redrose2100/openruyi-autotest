#!/bin/bash
# ============================================================
# K8s e2e custom focus: sig-storage (Sonobuoy)
#   Run `sonobuoy run --e2e-focus="\[sig-storage\]"` to verify
#   the whole storage SIG test surface on RISC-V K8s.
...
rlJournalStart
    ...
    rlPhaseStartTest "Run sonobuoy e2e focus sig-storage"
        ...
        rlRun "hwRunOnServer 1 \"$cmd\"" 0 "sonobuoy run completes for sig-storage"

        # Retrieve results tarball
        rlRun "hwRunOnServer 1 'sudo sonobuoy retrieve -f /tmp/k8s-sig-storage-results.tar.gz --kubeconfig=/etc/kubernetes/admin.conf'" 0 "Retrieve sonobuoy results"

        # Evaluate results
        rlRun "hwRunOnServer 1 'sudo sonobuoy results /tmp/k8s-sig-storage-results.tar.gz --mode=detailed'" 0 "sonobuoy results detailed runs"

        # Parse pass/fail counts and assert 100% pass rate
        summary=$(hwRunOnServer 1 "sudo sonobuoy results /tmp/k8s-sig-storage-results.tar.gz" 2>/dev/null)
        ...
        if [ -z "$total" ]; then
            ...
            if [ "${failed:-0}" -eq 0 ] && [ "$total" -ge 1 ]; then
                rlPass "sig-storage focused tests all passed ($passed/$total)"
            else
                rlFail "sig-storage has $failed failed test(s) out of $total"
            fi
        fi
    rlPhaseEnd
    ...
    rlPhaseStartCleanup "Cleanup: remove sonobuoy resources"
        ...
        rlRun "hwRunOnServer 1 'rm -f /tmp/k8s-sig-storage-results.tar.gz'" 0 "Remove local results tarball"
    rlPhaseEnd
rlJournalEnd
