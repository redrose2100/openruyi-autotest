#!/bin/bash
# ============================================================
# K8s conformance - api webhooks
#   Run `sonobuoy run --mode non-disruptive-conformance`
#   with an explicit --e2e-focus regex (26 tests).
...
rlJournalStart
    ...
    rlPhaseStartTest "Run sonobuoy conformance (26 tests)"
        ...
        focus='\[sig-api-machinery\].*(AdmissionWebhook|ValidatingAdmissionPolicy)'
        ...
        if [ -z "$detailed" ]; then
            ...
            if [ "$real_failures" -eq 0 ]; then
                rlPass "Conformance focus group passed (26 tests, no unsupported-classified failures)"
            else
