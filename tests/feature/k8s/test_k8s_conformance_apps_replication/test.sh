#!/bin/bash
# ============================================================
# K8s conformance - apps replication
#   Run `sonobuoy run --mode non-disruptive-conformance`
#   with an explicit --e2e-focus regex (12 tests).
...
rlJournalStart
    ...
    rlPhaseStartTest "Run sonobuoy conformance (12 tests)"
        ...
        focus='\[sig-apps\].*(ReplicationController|ReplicaSet)'
        ...
        if [ -z "$detailed" ]; then
            ...
            if [ "$real_failures" -eq 0 ]; then
                rlPass "Conformance focus group passed (12 tests, no unsupported-classified failures)"
            else
