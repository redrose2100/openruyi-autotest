#!/bin/bash
# ============================================================
# K8s conformance - network proxy apiserver
#   Run `sonobuoy run --mode non-disruptive-conformance`
#   with an explicit --e2e-focus regex (4 tests).
...
rlJournalStart
    ...
    rlPhaseStartTest "Run sonobuoy conformance (4 tests)"
        ...
        focus='\[sig-network\].*(Proxy|API Server)'
        ...
        if [ -z "$detailed" ]; then
            ...
            if [ "$real_failures" -eq 0 ]; then
                rlPass "Conformance focus group passed (4 tests, no unsupported-classified failures)"
            else
