#!/bin/bash
# ============================================================
# K8s conformance - node volumes env
#   Run `sonobuoy run --mode non-disruptive-conformance`
#   with an explicit --e2e-focus regex (25 tests).
...
rlJournalStart
    ...
    rlPhaseStartTest "Run sonobuoy conformance (25 tests)"
        ...
        focus='\[sig-node\].*(Variable Expansion|Downward API|ConfigMap|Secrets)'
        ...
        if [ -z "$detailed" ]; then
            ...
            if [ "$real_failures" -eq 0 ]; then
                rlPass "Conformance focus group passed (25 tests, no unsupported-classified failures)"
            else
