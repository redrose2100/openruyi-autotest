#!/bin/bash
# ============================================================
# K8s conformance - scheduling scheduler
#   Run `sonobuoy run --mode non-disruptive-conformance`
#   with an explicit --e2e-focus regex (11 tests).
...
rlJournalStart
    ...
    rlPhaseStartTest "Run sonobuoy conformance (11 tests)"
        ...
        focus='\[sig-scheduling\].*(Scheduler|LimitRange)'
        ...
        if [ -z "$detailed" ]; then
            ...
            if [ "$real_failures" -eq 0 ]; then
                rlPass "Conformance focus group passed (11 tests, no unsupported-classified failures)"
            else
