#!/bin/bash
# ============================================================
# K8s conformance - node security
#   Run `sonobuoy run --mode non-disruptive-conformance`
#   with an explicit --e2e-focus regex (8 tests).
...
rlJournalStart
    ...
    rlPhaseStartTest "Run sonobuoy conformance (8 tests)"
        ...
        focus='\[sig-node\].*(Security Context|Sysctls|NoExecuteTaintManager)'
        ...
        if [ -z "$detailed" ]; then
            ...
            if [ "$real_failures" -eq 0 ]; then
                rlPass "Conformance focus group passed (8 tests, no unsupported-classified failures)"
            else
