#!/bin/bash
# ============================================================
# K8s conformance - apps job
#   Run `sonobuoy run --mode non-disruptive-conformance`
#   with an explicit --e2e-focus regex (19 tests).
...
rlJournalStart
    ...
    rlPhaseStartTest "Run sonobuoy conformance (19 tests)"
        ...
        if [ -z "$detailed" ]; then
            ...
            if [ "$real_failures" -eq 0 ]; then
                rlPass "Conformance focus group passed (19 tests, no unsupported-classified failures)"
            else
