#!/bin/bash
# ============================================================
# K8s conformance - api namespaces
#   Run `sonobuoy run --mode non-disruptive-conformance`
#   with an explicit --e2e-focus regex (7 tests).
...
rlJournalStart
    ...
    rlPhaseStartTest "Run sonobuoy conformance (7 tests)"
        ...
        focus='\[sig-api-machinery\].*(Namespaces|OrderedNamespaceDeletion|namespace deletion)'
        ...
        if [ -z "$detailed" ]; then
            ...
            if [ "$real_failures" -eq 0 ]; then
                rlPass "Conformance focus group passed (7 tests, no unsupported-classified failures)"
            else
