#!/bin/bash
# ============================================================
# K8s conformance - architecture min nodes
#   Run `sonobuoy run --mode non-disruptive-conformance`
#   with an explicit --e2e-focus regex (1 tests).
...
rlJournalStart
    ...
    rlPhaseStartTest "Run sonobuoy conformance (1 tests)"
        ...
        focus='\[sig-architecture\].*Conformance Tests'
        ...
        if [ -z "$detailed" ]; then
            ...
            if [ "$real_failures" -eq 0 ]; then
                rlPass "Conformance focus group passed (1 tests, no unsupported-classified failures)"
            else
