#!/bin/bash
# ============================================================
# K8s conformance - apps pdb
#   Run `sonobuoy run --mode non-disruptive-conformance`
#   with an explicit --e2e-focus regex (5 tests).
...
rlJournalStart
    ...
    rlPhaseStartTest "Run sonobuoy conformance (5 tests)"
        ...
        focus='\[sig-apps\].*(DisruptionController|PodDisruptionBudget)'
        ...
        if [ -z "$detailed" ]; then
            ...
            if [ "$real_failures" -eq 0 ]; then
                rlPass "Conformance focus group passed (5 tests, no unsupported-classified failures)"
            else
