#!/bin/bash
# ============================================================
# K8s conformance - api discovery watch
...
rlJournalStart
    ...
    rlPhaseStartTest "Run sonobuoy conformance (17 tests)"
        ...
        focus='\[sig-api-machinery\].*(AggregatedDiscovery|Discovery|API chunking|Table transformation|server version|Watchers|API priority and fairness)'
        ...
        if [ -z "$detailed" ]; then
            ...
            if [ "$real_failures" -eq 0 ]; then
                rlPass "Conformance focus group passed (17 tests, no unsupported-classified failures)"
            else
