#!/bin/bash
# ============================================================
# K8s conformance - storage secrets subpath
#   Run `sonobuoy run --mode non-disruptive-conformance`
#   with an explicit --e2e-focus regex (14 tests).
...
rlJournalStart
    ...
    rlPhaseStartTest "Run sonobuoy conformance (14 tests)"
        ...
        focus='\[sig-storage\].*(Secret|Subpath)'
        ...
        if [ -z "$detailed" ]; then
            ...
            if [ "$real_failures" -eq 0 ]; then
                rlPass "Conformance focus group passed (14 tests, no unsupported-classified failures)"
            else
